...
set_mlst_coverage_scores <- set_mlst_coverage %>%
        group_by(set_name) %>%
        summarise(
                set_score = 0.75 * coverage_at_5000 + 0.25 * coverage_at_10000,
                score_function = "sum_{mlst schemes}{0.75 * frac_loci_covered_assuming_5000bp_extension + 0.25 * frac_loci_covered_assuming_10000bp_extension}") %>%
        arrange(desc(set_score))

write.table(
        x = set_mlst_coverage,
        file = paste(outdir, "mlst_loci_coverage/set_mlst_coverage.txt", sep = "/"),
        row.names = F
)
write.table(
        x = set_mlst_coverage_scores,
        file = paste(outdir, "mlst_loci_coverage/set_mlst_coverage_scores.txt", sep = "/"),
        row.names = F
)
```
...
write.table(
        ...
        file = paste(outdir, "genome_coverage/genome_coverage_by_set.txt", sep = "/"),
        row.names = F
)
```
